...
<?php add_action( 'wp_enqueue_scripts', 'readit_scripts' ); ?>
	...
	<header id="masthead" class="site-header" role="banner">
		<div class="site-branding">
        	...
        	<?php if ( get_theme_mod( 'readit_logo' ) ) : ?>  
    			<div class="site-logo"> 
       				<a href='<?php echo esc_url( home_url( '/' ) ); ?>' title='<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>' rel='home'><img src='<?php echo esc_url( get_theme_mod( 'readit_logo' ) ); ?>' <?php if ( get_theme_mod( 'logo_size' ) ) : ?>width="<?php echo esc_attr( get_theme_mod( 'logo_size' ), __( '120', 'readit' )); ?>"<?php endif; ?> alt='<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>'></a>  
    			</div><!-- site-logo --> 
			<?php else : ?>
    			<hgroup> 
